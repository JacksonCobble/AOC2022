...
void Day2()
{
    //initit point buckets, open input file
    int pointsP1 = 0;
    int pointsP2 = 0;
    std::string tmp;
    std::ifstream file("Input_Data/input2.txt");

    //map of possible combinations with resulting point values
    std::map<std::string, int> outcomes = { {"A X", 4}, {"A Y", 8}, {"A Z", 3},
                                            {"B X", 1}, {"B Y", 5}, {"B Z", 9},
                                            {"C X", 7}, {"C Y", 2}, {"C Z", 6}, };

    //map tells you what to play to beat your opponent
    std::map<char, std::string>win = { {'A', "Y"}, {'B', "Z"}, {'C', "X"} };
    //map tells you what to play to lose to your opponent
    std::map<char, std::string>loss = { {'A', "Z"}, {'B', "X"}, {'C', "Y"} };

    while (std::getline(file, tmp))
    {
        //use lookup table for P1
        pointsP1 += outcomes[tmp];
        //for P2, look at 3rd char to determine what we will try to do
        switch (tmp.at(2)) {
        case 'X': //we want to lose here, so find the char that loses against our opponent, build string, use lookup table
            pointsP2 += outcomes[std::string() + tmp.at(0) + tmp.at(1) + loss[tmp.at(0)]];
            break;
        case 'Y': //we want to tie here, so convert opponents hand to our hand with ascii addition, use lookup table
            pointsP2 += outcomes[std::string() + tmp.at(0) + tmp.at(1) + (char)(tmp.at(0) + 23)];
            break;
        case 'Z': //we want to win here, so find the char that wins against our opponent, build string, use lookup table
            pointsP2 += outcomes[std::string() + tmp.at(0) + tmp.at(1) + win[tmp.at(0)]];
            break;
        }
    }
    //output
    std::cout << "Day 2" << std::endl;
    std::cout << "part 1: " << pointsP1 << std::endl;
    std::cout << "part 2: " << pointsP2 << std::endl;
    std::cout << std::endl;
    return;
}